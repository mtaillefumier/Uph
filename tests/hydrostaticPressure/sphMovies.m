...
cd ('/home/kchola/simulations/hydrostaticPressure')
output = dir('out*.dat');
name = {output.name};
% sort the files
...
cn = sqrt(0.3*9.81);
en = 1000.0*9.81*0.3^3*(1-2.0*0.3/1.0);
for k = 1:numfiles
...
%      pause(0.0001);
     ...
     elseif(a==2) % animation using scatter to render the particles
         scatter(x,y,b,z/pn), colormap jet;%(bluewhitered(256));%colormap (mymap); 
